...
def BinarySTL(fname):
    ...
    x = np.hstack((Vertex1[:, 0][:, np.newaxis], Vertex2[:, 0][:, np.newaxis],
                   Vertex3[:, 0][:, np.newaxis])).flatten()
    y = np.hstack((Vertex1[:, 1][:, np.newaxis], Vertex2[:, 1][:, np.newaxis],
                   Vertex3[:, 1][:, np.newaxis])).flatten()
    z = np.hstack((Vertex1[:, 2][:, np.newaxis], Vertex2[:, 2][:, np.newaxis],
                   Vertex3[:, 2][:, np.newaxis])).flatten()
    """
...
def check_header_on_file(path, at_least=3):
    ...
    check_header_on_file(...)
        ...
        it gets to at_least number than it calculates the number of header
        rows.
...
def swap_load_ascii_single_grid(path, blocks, header=0, dtype='float32',
                                swap_directory='TMP'):
    '''
    ...
    while chosen_name in directory_list_of_files:
        chosen_name = 'grid_' + str(np.random.randint(1000, 9999)) + '.dat'
    mem_grid = np.memmap(swap_directory + '\\' + chosen_name, dtype=dtype,
                         mode='w+', shape=blocks, order='F')
...
def swap_load_ascii_multiple_grid(path, blocks, header=0, dtype='float32',
                                  columns=2, swap_directory='TMP'):
    '''
    swap_load_ascii_multiple_grid(...)
        swap_load_ascii_multiple_grid(path,blocks,header,dtype,columns,
        wap_directory)

        Loads a multiple (multiple columns) grid (mesh) ASCII file into a
        memory swap space.
    ...
    if dtype in ['float', 'float16', 'float32', 'float64']:
        ...
        for z in xrange(blocks[2]):
            for y in xrange(blocks[1]):
                for x in xrange(blocks[0]):
                    mem_grid[x, y, z, :] = np.bool_(
                        np.int_(fid.readline().split()))
    elif dtype in ['int', 'int8', 'int16', 'int32', 'int64', 'uint8', 'uint16',
                   'uint32', 'uint64']:
        for z in xrange(blocks[2]):
            for y in xrange(blocks[1]):
...
def load_ascii_multiple_grid(path, blocks, header=0, dtype='float32',
                             columns=2):
    '''
    ...
    for i in xrange(header):  # @UnusedVariable
        fid.readline()
    grid = np.zeros(
        (blocks[0], blocks[1], blocks[2], columns), dtype=dtype, order='F')
    if dtype in ['float', 'float16', 'float32', 'float64']:
        for z in xrange(blocks[2]):
            for y in xrange(blocks[1]):
                for x in xrange(blocks[0]):
...
def swap_load_npy_grid(path, swap_directory='TMP'):
    ...
    out: numpy swap array
        3D/4D numpy swap array with information on file (4th dimension is given
...
def cerena_save_grid_by_dictionary(dictionary, opath='default_grid_name.prn', fmt='%10.3f', header=False):
    ...
    if opath[-4:] == '.npy':
        ...
        for v in dictionary.keys():
            grid.append(
                dictionary[v].data.reshape((shape[0], shape[1], shape[2], 1)))
...
def cerena_save_flexible_data_by_dictionary(dictionary, opath='default_data_name.prn', fmt='%10.3f', header=False):
    ...
    if opath[-4:] == '.npy':
        ...
        for x in xrange(shape[0]):
            ...
            for v in dictionary.keys():
                if dictionary[v].dtype != 'string':
                    fid.write((fmt + '          ') % dictionary[v].data[x])
                else:
                    fid.write(dictionary[v].data.data[x] + '          ')
            fid.write('\n')
        fid.close()
        return True
    ...
    if opath[-4:] == '.npy':
        ...
        if type(header) != bool:
            if header[-1:] == '\n':
                ...
                fid.write(header + '\n')
        if type(point[0]) != str:
            np.savetxt(fid, point.data, fmt=fmt)
        else:
...
def save_indicator_block(block, opath='default_indicator_block_name.prn', fmt='%10.3f'):
    ...
    for k in xrange(block[1].shape[0]):
        fid.write('block #' + str(count) + '\n')
        for j in xrange(block[1].shape[1] - 1):
            fid.write('%i' % (block[1][k, j]) + ';')
...
def pygeo_get_point_names_from_header(path, columns, coordinate_columns=(1, 2, 3)):
    ...
    fid.readline()
    variables = []
    coordinate_columns = [coordinate_columns[0] - 1,
        coordinate_columns[1] - 1, coordinate_columns[2] - 1]
...
def convert_npy_to_ascii(ipath, opath, fmt='%10.3f'):
    ...
    if ipath[-4:] != '.npy':
        ...
        if len(idata.shape) > 4:
            ...
            fid = open(opath, 'w')
            fid.write(
                'your_data_' + str(idata.shape[0]) + '_' + str(idata.shape[1]) + '_' + str(idata.shape[2]) + '\n')
            fid.write('%i\n' % idata.shape[3])
            ...
            fid = open(opath, 'w')
            fid.write('your_data_' + str(idata.shape[0]) + '_' + str(
                idata.shape[1]) + '_' + str(idata.shape[2]) + '\n1\nvariable0\n')
            for z in xrange(idata.shape[2]):
                for y in xrange(idata.shape[1]):
                    for x in xrange(idata.shape[0]):
...
def convert_ascii_to_npy(ipath, opath, data_type='grid', dtype='float32', blocks=(1, 1, 1), at_least=3):
    ...
    if opath[-4:] != '.npy':
        ...
        if type(dtype) == bool:
            dtype = check_dtype_on_file(ipath, header, columns)
        if data_type == 'grid':
            if columns > 1:
                grid = np.zeros(
                    (blocks[0], blocks[1], blocks[2], columns), dtype=dtype)
                if dtype in ['float', 'float16', 'float32', 'float64']:
                    fid = open(ipath, 'r')
                    for i in xrange(header):
                        fid.readline()
                ...
                if dtype in ['float', 'float16', 'float32', 'float64']:
                    ...
                    if columns > 1:
                        grid = np.zeros(
                            (blocks[0], blocks[1], blocks[2]), dtype=dtype)
                        for z in xrange(blocks[2]):
                            for y in xrange(blocks[1]):
                                for x in xrange(blocks[0]):
                                    appex = fid.readline().replace('\n', '')
            ...
            grib = load_grib(
                ipath, dtype='float32', swap=False, swap_directory='TMP', at_least=3)
            np.save(opath, grib)
    return True
...
def swap_load_ascii_multiple_special_char_grid(path, blocks, header=0, dtype='float32', columns=2, sep=';', swap_directory='TMP'):
    ...
    mem_grid = np.memmap(swap_directory + '\\' + chosen_name, dtype=dtype,
                         mode='w+', shape=(blocks[0], blocks[1], blocks[2], columns), order='F')
    if dtype in ['float', 'float16', 'float32', 'float64']:
        for z in xrange(blocks[2]):
            for y in xrange(blocks[1]):
                for x in xrange(blocks[0]):
        ...
        for z in xrange(blocks[2]):
            for y in xrange(blocks[1]):
                for x in xrange(blocks[0]):
                    mem_grid[x, y, z, :] = np.bool_(
                        np.int_(fid.readline().split(sep)))
    elif dtype in ['int', 'int8', 'int16', 'int32', 'int64', 'uint8', 'uint16', 'uint32', 'uint64']:
        for z in xrange(blocks[2]):
            for y in xrange(blocks[1]):
                for x in xrange(blocks[0]):
...
def load_special_char_grid(path, blocks=(1, 1, 1), dtype='float32', swap=False, swap_directory='TMP', at_least=3):
    ...
    if path[-4:] == '.npy':
        ...
        if type(blocks) == tuple:
            ...
            sep = appex[2]
            if type(dtype) == bool:
                dtype = check_dtype_on_special_char_file(
                    path, header, columns, sep)
...
def load_special_char_data(path, dtype='float32', swap=False,
    ...
    if path[-4:] == '.npy':
        ...
        sep = appex[2]
        if type(dtype) == bool:
            dtype = check_dtype_on_special_char_file(path, header, columns,
                                                     sep)
        return load_ascii_special_char_data(path, header, dtype, sep, swap,
                                            swap_directory)
